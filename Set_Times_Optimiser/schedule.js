document.addEventListener('DOMContentLoaded', function() {
    ...
    calculateBtn.addEventListener('click', function() {
        ...
        // Validate inputs
        if (artists.some(artist => !artist.name || !artist.start || !artist.end)) {
    ...
    function findOptimalSchedule(artists, travelTime, missTime = 0) {
        ...
        const weightedArtists = artists.map(artist => ({
            ...artist,
        ...
        const sortedByEnd = [...weightedArtists].sort((a, b) => a.endMinutes - b.endMinutes);
        ...
        while (sortedByEnd.length > 0) {
            ...
            for (let i = 0; i < sortedByEnd.length; i++) {
                ...
                const requiredGap = (lastStage && artist.stage !== lastStage) ? travelTime : 0;
                
                // Calculate how much we'd need to miss (if any)
                const missAmount = Math.max(0, (lastEndTime + requiredGap) - artist.startMinutes);
            ...
            const actualStart = Math.max(selectedArtist.startMinutes, lastEndTime + bestGap);
            const missedMinutes = Math.max(0, (lastEndTime + bestGap) - selectedArtist.startMinutes);
    ...
    function displayResults(schedule, conflicts, travelTime) {
        ...
        stages.forEach(stage => {
            ...
            const stageArtists = schedule.filter(a => (a.stage || 'Main Stage') === stage);
    ...
    function calculateStats(schedule) {
        ...
        return {
            ...
            travelTime,
            missedTime,
            efficiency: totalAvailableTime > 0 
